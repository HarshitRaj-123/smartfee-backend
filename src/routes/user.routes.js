...
router.get('/by-role/:role', verifyToken, permit('ADMIN_MANAGEMENT'), async (req, res) => {
  try {
    ...
    // Validate role
    if (!['admin', 'accountant', 'student'].includes(role)) {
      return res.status(400).json({ 
        success: false, 
        message: 'Invalid role. Only admin, accountant, and student are allowed.' 
      });
    }
...
router.post('/:id/reset-password', verifyToken, permit('ADMIN_MANAGEMENT'), async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }
    // Set a default password (could be randomized or sent to user)
...
router.get('/profile', verifyToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching user profile', error: error.message, stack: error.stack });
  }
});
